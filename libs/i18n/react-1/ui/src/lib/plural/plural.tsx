...
export function Plural(props: PluralProps) {
  ...
  useEffect(() => {
    ...
    let target;
    if (message.hasOwnProperty('target') && message.hasOwnProperty('type') === "Plural") {
      switch (props.count) {
        case 0:
          target = message.target.hasOwnProperty('zero') ? message.target.zero : 'Not found';
          break;
        case 1:
          target = message.target.hasOwnProperty('one') ? message.target.one : 'Not found';
          break;
        case 2:
          target = message.target.hasOwnProperty('two') ? message.target.two : 'Not found';
          break;
        default:
          target = message.target.hasOwnProperty('other') ? message.target.other : 'Not found';
          break;
      }
    }
    else {
